...
--- Looks for a Templater-style `<%` marker or a leading ` ```js ` code block.
---
---@param template_content string
---@return boolean
M.has_templater_js = function(template_content)
  -- Check for Templater execution markers. All Templater tags start with a literal
  -- `<%` (`<%`, `<%=`, `<%#`, `<%*`, `<%-`, ...), so matching the literal `<%` covers them all.
  -- The `%` must be escaped as `%%` in a Lua pattern.
  if template_content:match "<%%" then
    return true
  end
...
M.run_templater = function(template_path, ctx)
  ...
  -- Build a flat argv list: { cmd, arg1, arg2, ... }. Passing a nested table to
  -- `vim.system` would drop the arguments entirely.
  local argv = { cmd }
  vim.list_extend(argv, args)

  -- Guard against both a non-zero exit (result.code ~= 0) and a missing binary,
  -- where `vim.system` raises an ENOENT error instead of returning a result table.
  ...
  if not ok then
    return nil, string.format("failed to run templater '%s': %s", cmd, tostring(result_or_err))
  end

  if result_or_err.code ~= 0 then
    return nil, string.format("templater exited with code %d: %s", result_or_err.code, result_or_err.stderr or "")
  end

  return result_or_err.stdout or "", nil
end
...
M.substitute_with_templater = function(template_path, ctx)
  ...
  -- Split output into lines without keeping the line terminators. `vim.split` with
  -- `plain = true` preserves empty interior lines (e.g. blank lines between frontmatter
  -- and body), which `gmatch` patterns can mangle.
